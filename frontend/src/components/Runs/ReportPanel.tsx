...
export const ReportPanel = ({ runId }: { runId: string }) => {
  const pdfMutation = useMutation({
    mutationFn: async () => {
      await runsService.generateReport(runId, "pdf");
      await runsService.downloadReport(runId, "pdf");
    },
  });

  const jsonMutation = useMutation({
    mutationFn: async () => {
      await runsService.generateReport(runId, "json");
      await runsService.downloadReport(runId, "json");
    },
  });
